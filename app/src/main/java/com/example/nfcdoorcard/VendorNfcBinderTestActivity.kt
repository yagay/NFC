...
/**
 * V6 direct diagnostic.
 *
 * The OEM metadata probe established that VendorNfcService exposes
 * getNfcAdapterVendorInterface() and that TRANSACTION_enableNfcShareMode == 15.
 * V6 checks whether the normal app-side INfcAdapter proxy exposes the same zero-argument
 * getter. If it does, the returned vendor Binder is used directly by this app UID for one
 * enter/verify/cleanup test. No root relay and no com.android.nfc-side relay is used.
 */
class VendorNfcBinderTestActivity : ComponentActivity() {
    companion object {
        private const val VENDOR_DESCRIPTOR = "com.vendor.nfc.IVendorNfcAdapter"
        private const val EXPECTED_TX_ENABLE_SHARE_MODE = 15
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = "Vendor NFC 直接测试 V6"

        output = TextView(this).apply {
            textSize = 12f
            setTextIsSelectable(true)
            text = "V6 会由 App 自己的 UID 精确查找 getNfcAdapterVendorInterface()。\n若返回 IVendorNfcAdapter Binder，则用已确认的 transaction 15 做一次 true → RF 验证 → false 清理。\n\n不会使用 root/NFC 进程代发，也不会猜 transaction code。"
        }

        runButton = Button(this).apply {
            text = "开始 Vendor NFC 直接测试 V6"
            setOnClickListener { runV6() }
        }
    ...
    private fun runV6() {
        ...
        executor.execute {
            val report = buildString {
                appendLine("=== VENDOR NFC DIRECT TEST V6 ===")
                ...
                val metadataTx = before["vendor_share_mode_transaction"]?.toIntOrNull()
                val metadataDesc = before["vendor_binder_descriptor"].orEmpty()

                appendLine("simulation_enabled=$simulationEnabled target_uid=$targetUid before_probe_events=$beforeEvents")
                appendLine("metadata_tx=$metadataTx metadata_descriptor=$metadataDesc")

                if (!simulationEnabled || targetUid.isBlank()) {
                    appendLine("RESULT=TEST_NOT_RUN reason=simulation_not_enabled")
                    return@buildString
                }
                if (metadataTx != EXPECTED_TX_ENABLE_SHARE_MODE) {
                    appendLine("RESULT=TEST_NOT_RUN reason=unexpected_transaction_code expected=$EXPECTED_TX_ENABLE_SHARE_MODE actual=$metadataTx")
                    return@buildString
                }
                if (metadataDesc != VENDOR_DESCRIPTOR) {
                    appendLine("RESULT=TEST_NOT_RUN reason=unexpected_vendor_descriptor actual=$metadataDesc")
                    return@buildString
                }
                ...
                val exactMethods = collectMethods(service.javaClass)
                    .filter { it.name == "getNfcAdapterVendorInterface" }
                    .distinctBy { it.toGenericString() }
                exactMethods.forEach { appendLine("GETTER_CANDIDATE=${it.toGenericString()}") }

                val zeroArgGetter = exactMethods.firstOrNull { it.parameterCount == 0 }
                if (zeroArgGetter == null) {
                    collectMethods(service.javaClass)
                        .filter { it.name.contains("vendor", ignoreCase = true) || it.returnType.name.contains("vendor", ignoreCase = true) }
                        .distinctBy { it.toGenericString() }
                        .take(40)
                        .forEach { appendLine("INFC_VENDOR_METHOD=${it.toGenericString()}") }
                    appendLine("RESULT=TEST_FAIL stage=VENDOR_GETTER reason=zero_arg_getNfcAdapterVendorInterface_not_exposed_on_app_INfcAdapter")
                    return@buildString
                }

                val vendorBinder = try {
                    zeroArgGetter.isAccessible = true
                    val returned = zeroArgGetter.invoke(service)
                ...
                runCatching {
                    contentResolver.insert(ConfigProvider.URI, ContentValues().apply {
                        put(ConfigProvider.KEY_RF_STATUS, "WAITING_BINDER_TEST_V6")
                ...
                try {
                    enterAccepted = transactShareMode(vendorBinder, metadataTx, true, this)
                    ...
                    try {
                        appendLine("enableNfcShareMode(false)_cleanup=${transactShareMode(vendorBinder, metadataTx, false, this)}")
                    } catch (t: Throwable) {
            ...
            AppLogger.i("VENDOR_DIRECT_TEST_V6:\n$report")
            runCatching { File(cacheDir, "vendor_nfc_direct_test_v6.txt").writeText(report) }
            runOnUiThread {
                output.text = report
    ...
    private fun transactShareMode(binder: IBinder, code: Int, enabled: Boolean, log: StringBuilder): Boolean? {
